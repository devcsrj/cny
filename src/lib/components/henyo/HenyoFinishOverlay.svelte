<script lang="ts">
	...
	let formattedTime = $derived.by(() => {
		const totalSeconds = Math.floor(game.timeTakenMs / 1000);
...
>
	...
	<div class="mb-12 flex flex-col items-center gap-4">
		<div class="flex flex-col items-center gap-1">
			<div class="text-xl font-black tracking-widest uppercase opacity-70">Words Guessed</div>
			<div class="text-7xl leading-none font-black md:text-8xl">{game.score}</div>
		</div>
		<div class="flex flex-col items-center gap-1">
			<div class="text-xl font-black tracking-widest uppercase opacity-70">Time Taken</div>
			<div class="text-5xl leading-none font-black md:text-6xl">{formattedTime}</div>
		</div>
	</div>

	<div class="grid w-full max-w-4xl grid-cols-1 gap-8 md:grid-cols-2 md:gap-16">
		<!-- Correct Words -->
		<div class="flex flex-col gap-4">
			<h2
				class="flex items-center justify-between border-b-4 border-white pb-2 text-xl font-black tracking-widest uppercase"
			>
				Correct
				<span class="rounded bg-white px-2 py-0.5 text-sm text-primary"
			...
			{:else}
				<p class="font-bold text-white/50 uppercase italic">None</p>
			{/if}
		</div>
		...
		<div class="flex flex-col gap-4">
			<h2
				class="flex items-center justify-between border-b-4 border-white/30 pb-2 text-xl font-black tracking-widest uppercase"
			>
				Missed
